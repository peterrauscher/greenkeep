...
export function planCommits(
  ...
  return days
    .map((date) => ({
      date,
      commits: Math.min(MAX_COMMITS_PER_DAY, counts[date] ?? 0),
    }))
    .filter((d) => d.commits > 0);
}
...
export function generateBashScript(opts: {
  ...
  const lines: string[] = [
    "#!/usr/bin/env bash",
    "# Graph Copier — backdated empty commits for a private GitHub repo.",
    "# Usage: bash graph-copy.sh",
    ...
    "",
    'DIR="$(mktemp -d /tmp/graph-copy.XXXXXX)"',
    'cd "$DIR"',
    "git init -b main",
    'echo "# $REPO" > README.md',
    'echo "Private contribution mirror generated by Graph Copier." >> README.md',
    "git add README.md",
    'git commit -m "chore: init contribution mirror"',
    "",
  ];
...
export function intensityLabel(mode: IntensityMode): string {
  ...
  if (mode === "levels") return "Match graph intensity";
  return "Match contribution counts";
}
